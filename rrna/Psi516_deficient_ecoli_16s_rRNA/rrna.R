files <- list.files(path = "./event/Psi516_deficient_ecoli_16s_rRNA/", pattern = ".tsv", full.names = T)
table <- list()
for (f in files){
  t <- read.delim(f, header = F, stringsAsFactors = F)
  n <- sapply(strsplit(unlist(strsplit(f, split = ".sm.forward.tsv")), split = "./event/Psi516_deficient_ecoli_16s_rRNA//"), function(x) x[2])
...
system("samtools view ./fastq/Psi516_deficient_ecoli_16s_rRNA.sorted.bam | awk '{ print $1, $3, $5}' > ./fastq/Psi516_deficient_ecoli_16s_rRNA_sam.txt")
sam <- read.delim("./fastq/Psi516_deficient_ecoli_16s_rRNA_sam.txt", header = F, stringsAsFactors = F, sep = " ", quote = "")
colnames(sam) <- c("id", "contig", "mapq")
sam$id <- unlist(strsplit(sam$id, split = "_Basecall_1D_template:1D_000:template"))
sam <- sam[sam$id %in% names(table), ]
#sam file

fastq <- read.delim("./fastq/Psi516_deficient_ecoli_16s_rRNA.fastq", header = F, stringsAsFactors = F, sep = " ", quote = "")
...
fastq$id <- unlist(strsplit(fastq$id, split = "_Basecall_1D_template:1D_000:template"))
...
save(table, file = "Psi516_deficient_ecoli_16s_rRNA_table.rda")
