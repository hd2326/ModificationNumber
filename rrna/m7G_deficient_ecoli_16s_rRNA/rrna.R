files <- list.files(path = "./event/m7G_deficient_ecoli_16s_rRNA/", pattern = ".tsv", full.names = T)
table <- list()
for (f in files){
  t <- read.delim(f, header = F, stringsAsFactors = F)
  n <- sapply(strsplit(unlist(strsplit(f, split = ".sm.forward.tsv")), split = "./event/m7G_deficient_ecoli_16s_rRNA//"), function(x) x[2])
...
system("samtools view ./fastq/m7G_deficient_ecoli_16s_rRNA.sorted.bam | awk '{ print $1, $3, $5}' > ./fastq/m7G_deficient_ecoli_16s_rRNA_sam.txt")
sam <- read.delim("./fastq/m7G_deficient_ecoli_16s_rRNA_sam.txt", header = F, stringsAsFactors = F, sep = " ", quote = "")
...
fastq <- read.delim("./fastq/m7G_deficient_ecoli_16s_rRNA.fastq", header = F, stringsAsFactors = F, sep = " ", quote = "")
...
save(table, file = "m7G_deficient_ecoli_16s_rRNA_table.rda")
